...
<template>
  <Teleport to="body">
    <div v-show="active" :id="menuId" ref="menu" class="absolute min-w-[8rem]">
      <div
        class="rounded-lg border border-gray-300 bg-gray-100 shadow-lg dark:border-neutral-700 dark:bg-neutral-800"
      >
        <div v-for="(op, index) in contextOptions" :key="index">
          <hr v-if="op.name == '|'" class="border-gray-300 dark:border-neutral-700" />
          <button
            v-else
            class="m-1 w-[calc(100%_-_8px)] rounded-md px-2 py-1 text-left text-sm text-gray-700 hover:bg-gray-200 active:bg-gray-300 dark:text-neutral-50 dark:hover:bg-neutral-700/70 dark:active:bg-neutral-900/70"
            :class="{
              'hover:border hover:border-red-500 hover:text-red-500 dark:hover:border-red-500 dark:hover:text-red-500':
                op.name == 'Delete',
